...
@section('title')
	{{ __('Withdrawal Series List') }}
@endsection

@section('content')
@php
	$withdrawalSeriess = \DB::table('withdrawal_series')->orderBy('to', 'DESC')->first();
	$y = 0;

	if ($withdrawalSeriess !== null) {
	    $y = (int) $withdrawalSeriess->to;
	} else {
		...
		<div class="row">
			<div class="col-md-12">
				<div class="header">
					<h4 class="title">
						{{ __('ASSIGNED LIST') }}
						<a href="{{ route('withrawal.series') }}" class="btn btn-primary text-primary">
...
@section('scriptss')
    ...
    $(document).ready(function () {
      assigned_series = jQuery('#assigned_series').DataTable({
        ...
        ajax: {
          url: "{{ route('withrawal.series.list') }}",
          type: "GET",
    ...
    $(document).on('click', '#refresh', function(e) {
      ...
      var nurl = "/withdrawal/series/list/all/ws/rf";
    ...
    let typingTimer; 
		  ...
		  series_from = param_series_from;
			...
			if (x > y) {
				...
				if(series_from == null || series_from == '') {
					...
					// 	newurl = "/withdrawal/series/list/all/ws/" + series_from + "/0/" + series_location + "/" + series_department;
					...
					var nurl = "/withdrawal/series/list/all/ws";
					assigned_series.ajax.url(nurl).load();
					console.log(1)
				}
					...
					if((series_location != null || series_location != '') && (series_department != null || series_department != '')) {
						newurl = "/withdrawal/series/list/all/ws/" + series_from + "/" + series_to + "/" + series_location + "/" + series_department;
						assigned_series.ajax.url(newurl).load();
					}
					else if((series_location != null || series_location != '') && (series_department == null || series_department == '')) {
						newurl = "/withdrawal/series/list/all/ws/" + series_from + "/" + series_to + "/" + series_location;
						assigned_series.ajax.url(newurl).load();
					}
					else if((series_location == null || series_location == '') && (series_department != null || series_department != '')) {
						newurl = "/withdrawal/series/list/all/ws/" + series_from + "/" + series_to + "/null/" + series_department;
						assigned_series.ajax.url(newurl).load();
					}
				}
				{{--
		...
		function toDoneTyping(param_series_to = 0) {
			...
			if(($('#series_to').val() == null || $('#series_to').val() == '' || $('#series_to').val() == 0) && ($('#series_from').val() == null || $('#series_from').val() == '' || $('#series_from').val() == 0)) {
				...
				if(series_location == null && series_department == null) {
					newurl = "/withdrawal/series/list/all/ws/" + series_from + "/" + series_to;
					assigned_series.ajax.url(newurl).load();
				}
				else {
					if((series_location != null || series_location != '') && (series_department != null || series_department != '')) {
						...
						newurl = "/withdrawal/series/list/all/ws/" + series_from + "/" + series_to + "/" + series_location + "/0";
						assigned_series.ajax.url(newurl).load();
					}
					else if((series_location == null || series_location == '') && (series_department != null || series_department != '')) {
						newurl = "/withdrawal/series/list/all/ws/" + series_from + "/" + series_to + "/0/" + series_department;
						assigned_series.ajax.url(newurl).load();
					}
				}
			}
		...
		$('#series_location').change(function() {
			...
			{
				newurl = "/withdrawal/series/list/location/" + series_location;
				assigned_series.ajax.url(newurl).load();
				console.log(newurl)
			}
			else if ((series_from == null || series_from == '') && (series_to == null || series_to == '') && (series_department != '' || series_department != null))
			{
				newurl = "/withdrawal/series/list/all/ws/0/0/" + series_location + "/" + series_department;
				assigned_series.ajax.url(newurl).load();
				console.log(newurl) 
			}
			else if ((series_from != null || series_from != '') && (series_to != null || series_to != '') && (series_department != '' || series_department != null))
		...
		$('#series_department').change(function() {
			...
			if ((series_from == null || series_from == '') && (series_to == null || series_to == ''))
			{
				newurl = "/withdrawal/series/list/department/" + series_department;
				assigned_series.ajax.url(newurl).load();
				console.log(newurl)
			}
